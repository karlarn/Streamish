...
up.Name, up.Email, up.ImageUrl, up.DateCreated as 'UserDate'
                            FROM Video v
...
c.Id, c.Message, c.UserProfileId
        ...
        public List<Video> GetHottest(string date)
        {
            using (var conn = Connection)
            {
                conn.Open();
                using (var cmd = conn.CreateCommand())
                {
                    var sql = @"Select * From Video Where DateCreated >= @Criterion";

                    cmd.CommandText = sql;
                    DbUtils.AddParameter(cmd, "@Criterion", date);
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
